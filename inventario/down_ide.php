...
$client = new nusoap_client($ws,true);
	...
	if ($sError) {
		...
		if($error<>0){
				...
				$empresas = $client->call('ItsGetData', array('UserSession' => $session, 'ItsClassName' => 'ERP_IDENTIFICADORES', 'RecordCount' => '-1', 'SQLFilter'=>"", 'SQLSort'=> '') );

				$ItsGetDataResult = $empresas["ItsGetDataResult"];
				$DataEmpresas = $empresas["XMLData"];
				$file = fopen("archivo.xml", "a");
				fwrite($file, $DataEmpresas . PHP_EOL);
				fclose($file);
					...
					for ($i=0; $i<sizeof($langs); $i++){
							if($count == 1){
								...
								$datos = array('IDD'=>$langs['@attributes']['ID'],
                                               ...
                                               'FK_ERP_PAISES'=>$langs['@attributes']['FK_ERP_PAISES'],
                                               'Z_FK_ERP_PAISES'=>$langs['@attributes']['Z_FK_ERP_PAISES'],
                                               'FEC_FAB'=>$langs['@attributes']['FEC_FAB'],
                                               'FEC_VEN'=>$langs['@attributes']['FEC_FAB'],
                                               'FK_ERP_EST_IDE'=>$langs['@attributes']['FK_ERP_EST_IDE'],
                                               'Z_FK_ERP_EST_IDE'=>$langs['@attributes']['Z_FK_ERP_EST_IDE']);
							}else{
							    ...
							    $datos = array('IDD'=>$langs[$i]['@attributes']['ID'],
                                               ...
                                               'FK_ERP_PAISES'=>$langs[$i]['@attributes']['FK_ERP_PAISES'],
                                               'Z_FK_ERP_PAISES'=>$langs[$i]['@attributes']['Z_FK_ERP_PAISES'],
                                               'FEC_FAB'=>$langs[$i]['@attributes']['FEC_FAB'],
                                               'FEC_VEN'=>$langs[$i]['@attributes']['FEC_VEN'],
                                               'FK_ERP_EST_IDE'=>$langs[$i]['@attributes']['FK_ERP_EST_IDE'],
                                               'Z_FK_ERP_EST_IDE'=>$langs[$i]['@attributes']['Z_FK_ERP_EST_IDE']);
                                }									
									$salida[] = $datos;
								}
